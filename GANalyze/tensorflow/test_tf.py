...
iters = 100 #number of clone images to generate (both positive and negative)
...
for y in range(num_categories):
    ...
    for feed_dict in feed_dicts:
        ...
        if opts["mode"]=="iterative":
            ...
            ims_batch.append(x) ### HIER KAN JE DE AFBEELDINGEN OPSLAAN ZONDER DE SCORE
    ...
    # save every image separately with a filename including category number, seed number, image number, alpha value, and score(???)
    alpha = step_sizes[0]
    alpha_values = list(np.arange((-iters * alpha), ((iters * alpha)+0.0000001), alpha))

    ims_final = ims
    ims_final = np.reshape(ims_final, (ims.shape[0] * ims.shape[1], ims.shape[2], ims.shape[3], ims.shape[4]))
    os.makedirs(result_dir, exist_ok=True)
    i = 0  # 10 sets per category
    j = 0  # 'iters' images per seed
    for im in ims_final:
        I = PIL.Image.fromarray(np.uint8(im), 'RGB')
        # print(outscores_batch)
        filename = ("cat" + str(y) + "_seed" + str(i) + "_img" + str(j) + #" a=(" +
                    #str(format(alpha_values[j], ".4f")) + #")__score=(" +
                    # str(outscores_batch[j][0]) + str(outscores_batch[j][1]) + ")
                    #")
                    ".jpg")
        I.save(os.path.join(result_dir, filename))
        if j < (iters*2):
            j += 1
        else:
            j = 0
